import React from 'react'

const Why = () => {
  return (
    <section className="bg-white dark:bg-gray-900">
  <div className="container px-6 py-10 mx-auto">
    <div className="lg:flex lg:items-center">
      <div className="w-full space-y-12 lg:w-1/2 ">
        <div>
          <h2 className="text-2xl font-semibold text-gray-800 capitalize lg:text-3xl dark:text-white">
          Why Choose SRS Wizard?
          </h2>
          <div className="mt-2">
            <span className="inline-block w-40 h-1 bg-blue-500 rounded-full" />
            <span className="inline-block w-3 h-1 ml-1 bg-blue-500 rounded-full" />
            <span className="inline-block w-1 h-1 ml-1 bg-blue-500 rounded-full" />
          </div>
        </div>
        <div className="md:flex md:items-start md:-mx-4">
         
          <div className="mt-4 md:mx-4 md:mt-0">
            <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
            Time Efficiency
            </h1>
            <p className="mt-3 text-gray-500 dark:text-gray-300">
            Get a detailed SRS document in minutes, not hours.
            </p>
          </div>
        </div>

        <div className="md:flex md:items-start md:-mx-4">
         
          <div className="mt-4 md:mx-4 md:mt-0">
            <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
            Precision
            </h1>
            <p className="mt-3 text-gray-500 dark:text-gray-300">
            Our AI understands your inputs and generates highly accurate SRS documents.
            </p>
          </div>
        </div>

        <div className="md:flex md:items-start md:-mx-4">
         
          <div className="mt-4 md:mx-4 md:mt-0">
            <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
            Customization
            </h1>
            <p className="mt-3 text-gray-500 dark:text-gray-300">
            Tailor the document to your specific needs with ease.
            </p>
          </div>
        </div>

        <div className="md:flex md:items-start md:-mx-4">
         
          <div className="mt-4 md:mx-4 md:mt-0">
            <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
            Collaboration
            </h1>
            <p className="mt-3 text-gray-500 dark:text-gray-300">
            Share the document with your team or clients effortlessly.
            </p>
          </div>
        </div>

        <div className="md:flex md:items-start md:-mx-4">
         
          <div className="mt-4 md:mx-4 md:mt-0">
            <h1 className="text-xl font-semibold text-gray-700 capitalize dark:text-white">
            Compatibility
            </h1>
            <p className="mt-3 text-gray-500 dark:text-gray-300">
            Export in popular formats like PDF, Word, or Google Docs.
            </p>
          </div>
        </div>
       
      </div>
      <div className="hidden lg:flex lg:items-center lg:w-1/2 lg:justify-center">
        <img
          className="w-[28rem] h-[28rem] object-cover xl:w-[34rem] xl:h-[34rem] rounded-full"
          src="https://images.unsplash.com/photo-1507238691740-187a5b1d37b8?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=755&q=80"
          alt=""
        />
      </div>
    </div>
   
  </div>
</section>

  )
}

export default Why